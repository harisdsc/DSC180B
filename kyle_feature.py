...
def get_stability_features(group):
    ...
    """
    daily = group.set_index('posted_date')['amount'].resample('D').sum().fillna(0)
    signal = daily.values
    n = len(signal)
...
def create_features(consDF, full_balance_df):
    ...
    A_exp = full_balance_df[full_balance_df["credit_or_debit"] == 'DEBIT'].copy()
    ...
    inc_meanie = A_inc.groupby('prism_consumer_id')['amount'].mean().reset_index().rename(columns={'amount': 'inc_mean'})
    exp_meanie = A_exp.groupby('prism_consumer_id')['amount'].mean().reset_index().rename(columns={'amount': 'exp_mean'})
